...
for i in range(2019, 2021, 1):
    ...
    i = 1
    print(
        f"Loading list of movies and getting their ids... Total Pages:{total_pages}")
...
for m_id in movies:
    ...
    try:
        ...
        movie['production_companies'] = list(
            map(lambda m: m['name'], result['production_companies']))
        movie['production_countries'] = list(
            map(lambda m: m['name'], result['production_countries']))
